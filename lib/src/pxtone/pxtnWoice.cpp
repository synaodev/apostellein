...
bool pxtnWoice::Voice_Allocate( int32_t voice_num )
{
	...
	for( int32_t i = 0; i < voice_num; i++ )
	{
		...
		p_vc->p_pcm       = new (std::nothrow) pxtnPulse_PCM  ();
		p_vc->p_ptn       = new (std::nothrow) pxtnPulse_Noise();
#ifdef  pxINCLUDE_OGGVORBIS
		p_vc->p_oggv      = new (std::nothrow) pxtnPulse_Oggv ();
#endif
		memset( &p_vc->envelope, 0, sizeof(pxtnVOICEENVELOPE) );
		if (!p_vc->p_pcm || !p_vc->p_ptn) {
			goto End;
		}
#ifdef  pxINCLUDE_OGGVORBIS
		if (!p_oggv) {
...
static void _UpdateWavePTV( pxtnVOICEUNIT* p_vc, pxtnVOICEINSTANCE* p_vi, int32_t  ch, int32_t, int32_t  bps )
{
	double  work, osc;
	int32_t long_;
