...
void _set_ocsillator( _OSCILLATOR *p_to, pxNOISEDESIGN_OSCILLATOR *p_from, int32_t sps, const short *p_tbl, const short *p_tbl_rand )
{
	...
	p_to->incriment  = ( _BASIC_SPS / sps ) * ( (double)p_from->freq   / _BASIC_FREQUENCY );

	// offset
	if( p_to->ran_type != _RANDOM_None ) p_to->offset = 0;
	else                                 p_to->offset = (double) _smp_num * ( (double)p_from->offset / _BASIC_FREQUENCY );

	p_to->volume     = (double)p_from->volume / 100.0;
	p_to->p_smp      = p_tbl;
	p_to->bReverse   = p_from->b_rev;

	p_to->rdm_start  = 0;
	p_to->rdm_index  = (int32_t)( (double)(_smp_num_rand) * ( (double)p_from->offset / _BASIC_FREQUENCY ) );
	p = p_tbl_rand;
...
bool pxtnPulse_NoiseBuilder::Init()
{
	...
	_freq = new (std::nothrow) pxtnPulse_Frequency(); if(!_freq || !_freq->Init() ) goto End;
...
pxtnPulse_PCM *pxtnPulse_NoiseBuilder::BuildNoise( pxtnPulse_Noise *p_noise, int32_t ch, int32_t sps, int32_t bps ) const
{
	...
	p_pcm = new (std::nothrow) pxtnPulse_PCM();
	if (!p_pcm) goto End;
	...
	for( int32_t s = 0; s < smp_num; s++ )
	{
		for( int32_t c = 0; c < ch; c++ )
		{
			...
			if( byte4 < -_SAMPLING_TOP ) byte4 = -_SAMPLING_TOP;
			if( bps ==  8 ){ *           p   = (unsigned char)( ( (unsigned short)byte4 >> 8 ) + 128 ); p += 1; } //  8bit
		...
		for( int32_t u = 0; u < unit_num; u++ )
		{
			...
			if( pU->bEnable )
			{
				...
				_incriment( &pU->main, pU->main.incriment * (double)_freq->Get( (int32_t)fre ), _p_tables[ pxWAVETYPE_Random ] );
