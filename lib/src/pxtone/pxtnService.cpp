...
pxtnERR pxtnService::_init( int32_t fix_evels_num, bool b_edit )
{
	...
	if( !( text      = new (std::nothrow) pxtnText              () ) ){ res = pxtnERR_INIT    ; goto End; }
	if( !( master    = new (std::nothrow) pxtnMaster            () ) ){ res = pxtnERR_INIT    ; goto End; }
	if( !( evels     = new (std::nothrow) pxtnEvelist           () ) ){ res = pxtnERR_INIT    ; goto End; }
	if( !( _ptn_bldr = new (std::nothrow) pxtnPulse_NoiseBuilder() ) ){ res = pxtnERR_INIT    ; goto End; }
...
bool pxtnService::Delay_Add( DELAYUNIT unit, float freq, float rate, int32_t group )
{
	...
	_delays[ _delay_num ] = new (std::nothrow) pxtnDelay();
	if (!_delays[ _delay_num ]) return false;
...
bool pxtnService::OverDrive_Add( float cut, float amp, int32_t group )
{
	...
	_ovdrvs[ _ovdrv_num ] = new (std::nothrow) pxtnOverDrive();
	if (!_ovdrvs[ _ovdrv_num ]) return false;
...
pxtnERR pxtnService::Woice_read( int32_t idx, pxtnDescriptor* desc, pxtnWOICETYPE type )
{
	...
	if( idx == _woice_num ){
		_woices[ idx ] = new (std::nothrow) pxtnWoice();
		if (!_woices[ idx ]) return pxtnERR_memory;
		_woice_num++;
	}
...
bool pxtnService::Unit_AddNew()
{
	...
	_units[ _unit_num ] = new (std::nothrow) pxtnUnit();
	if (!_units[ _unit_num ]) return false;
	_unit_num++;
	return true;
}
...
bool pxtnService::clear()
{
	...
	for( int32_t i = 0; i < _ovdrv_num; i++ ) SAFE_DELETE( _ovdrvs[ i ] ); _ovdrv_num = 0;
...
pxtnERR pxtnService::_io_Read_Delay( pxtnDescriptor *p_doc )
{
	...
	pxtnERR    res   = pxtnERR_VOID;
	pxtnDelay* delay = new (std::nothrow) pxtnDelay();
	if (!delay) {
		goto term;
	}
...
pxtnERR pxtnService::_io_Read_OverDrive( pxtnDescriptor *p_doc )
{
	...
	pxtnERR    res   = pxtnERR_VOID;
	pxtnOverDrive *ovdrv = new (std::nothrow) pxtnOverDrive();
	if (!ovdrv) {
		goto term;
	}
...
pxtnERR pxtnService::_io_Read_Woice( pxtnDescriptor *p_doc, pxtnWOICETYPE type )
{
	...
	pxtnWoice *woice = new (std::nothrow) pxtnWoice();
	if (!woice) goto term;
...
pxtnERR pxtnService::_io_Read_OldUnit( pxtnDescriptor *p_doc, int32_t ver        )
{
	...
	pxtnERR   res   = pxtnERR_VOID  ;
	pxtnUnit* unit  = new (std::nothrow) pxtnUnit();
	int32_t   group =              0;
	if (!unit) {
		goto term;
	}
		...
		case _TAG_num_UNIT    :
			{
				...
				for( int32_t i = 0; i < num; i++ ) {
					_units[ i ] = new (std::nothrow) pxtnUnit();
					if (!_units[ i ]) {
						goto term;
					}
				}
