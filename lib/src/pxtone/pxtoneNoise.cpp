...
{
	pxtnPulse_NoiseBuilder *bldr = new (std::nothrow) pxtnPulse_NoiseBuilder();
	if( !bldr || !bldr->Init() ){ delete bldr; bldr = NULL; return false; }
	_bldr = bldr;
	return true;
}
...
bool pxtoneNoise::generate( pxtnDescriptor *p_doc, void **pp_buf, int32_t *p_size ) const
{
	...
	pxtnPulse_Noise        *noise = new (std::nothrow) pxtnPulse_Noise();
