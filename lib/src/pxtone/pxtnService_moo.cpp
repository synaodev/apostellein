...
bool pxtnService::_moo_init()
{
	...
	if( !(_moo_freq = new (std::nothrow) pxtnPulse_Frequency()) ||  !_moo_freq->Init() ) goto term;
...
bool pxtnService::_moo_PXTONE_SAMPLE( void *p_data )
{
	...
	for( ; _moo_p_eve && _moo_p_eve->clock <= clock; _moo_p_eve = _moo_p_eve->next )
	{
		...
		case EVENTKIND_TUNING: {
			...
			float t = 0.0f;
			memcpy(&t, &_moo_p_eve->value, sizeof(t));
			p_u->Tone_Tuning(t);
			break;
		}
		}
...
bool pxtnService::moo_preparation( const pxtnVOMITPREPARATION *p_prep )
{
	...
	_moo_clock_rate = (float)( 60.0 * (double)_dst_sps / ( (double)_moo_bt_tempo * (double)_moo_bt_clock ) );
	_moo_smp_stride = ( 44100.0f / _dst_sps );
	_moo_top        = 0x7fff;

	_moo_time_pan_index = 0;

	_moo_smp_end    = (int32_t)( (double)meas_end    * (double)_moo_bt_num * (double)_moo_bt_clock * (double)_moo_clock_rate );
	_moo_smp_repeat = (int32_t)( (double)meas_repeat * (double)_moo_bt_num * (double)_moo_bt_clock * (double)_moo_clock_rate );
	...
	else                   { _moo_smp_start = (int32_t)( (double)start_meas  * (double)_moo_bt_num * (double)_moo_bt_clock * (double)_moo_clock_rate ); }
